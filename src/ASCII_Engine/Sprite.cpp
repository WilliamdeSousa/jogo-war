...
Sprite::Sprite(std::string nameFile, COR::Cor cor, bool transparent) : SpriteBase(cor, transparent)
{
	this->loadFromFile(nameFile);
}

Sprite::Sprite(std::ifstream &fsprt, int n, COR::Cor cor, bool transparent) : SpriteBase(cor, transparent)
{
	...
	for (int i = 0 ; i < sprt.getAltura() ; i++)
	{
        ...
        for (int si = sprt.getLimits()[i].front ; si <= sprt.getLimits()[i].end ; si++) {
			if (c + si >= 0 && c + si < this->limits[i].largLinha) {
                if (sprt.getTransparent() && sprt.getLinha(i)[si] == ' ') {
                    continue;
                }
